...
async function readProfilePermissionsFlexible(db:any){ try{ const {data,error}=await db.from('profile_permissions').select('perfil, permission'); if(!error && Array.isArray(data)){ const by:Record<string,string[]>{} = {}; for(const r of data as any[]){ const p=r.perfil||'unknown'; const perm=r.permission||''; if(!perm) continue; (by[p]=by[p]||[]).push(perm); } if(Object.keys(by).length) return by; } }catch{}
